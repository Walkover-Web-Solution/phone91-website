        ...
        <div class="wrapper clear pr">
         ...
         <?php if(isset($_REQUEST['fbgl']) && $_REQUEST['fbgl'] == 1) { ?>  
              ...
              <div class="fbglStep">
                  ...
                  <div class="stepZero cmn " style="">
                    <div>
                        <div style="padding:40px 0 20px" class="wrapper clear">
                            <h2>We need to add calling balance in your account</h2>
                        </div>

                        <!--<form name="fbCurrencyForm" id="fbCurrencyForm" method="POST" onsubmit="return false;" >--> 
                                ...
                                <aside class="boxsize">
                                    <div class="pdR3">
                                        <div class="verifyform">
                                            ...
                                            <input type="hidden" name="currencyId" id="currId" value="" />
                                            ...
                                            <button type="button" onclick="selectCurrency()" class="large googleplus btdn"> Done </button>
                                        </div>
                                    </div>
                                </aside>
                                <aside class="last">
                    ...
                    <div class="stepOne cmn" style="display:none">
...
//                mobNo = mobNo;
                $.ajax({
                        type: "POST",
                        url: "action.php", 
                        data: {"action": "checkNumber", 'number': mobNo,'countryCode':countryCode,'id':eid}, 
                        dataType: 'json',
                    success: function(response)
                    {
                        ...
                        {
                            if(response.message.code == "8007"){
                                 $('.stepOne').hide();
                                 $('.stepTwo').show();
                                 isSignupFlag = true;
                            }
                            else{
                                console.log(countryCode+mobNo);
                                $('#uname').val(countryCode+mobNo);
                                $('#searchWraps').hide();
                                $('#formDiv,#forgot').show();
                            }
                        }
                            ...
                            if(response.message.code == '4033'){
                             $('.stepOne').hide();
                             $('.stepTwo').show();
//                                   window.location = 'http://voice.phone91.com/controller/signUpController.php?call=fbGlSignUp&eid='+eid+'&domain='+window.location.host;
                            }else{
...
//                mobNo = countryCode+mobNo;
                $.ajax({
                        type: "POST", 
                        url: "action.php", 
                        data: {"action": "verifyConfirmationApi",'countryCode':countryCode ,'number': mobNo,'code':code}, 
                        dataType: 'json',
                    success: function(response)
                    {
                        console.log(response);
...
//                            $('#formDiv').show();
            ...
            function signUpwithApi()
            {
                var tempId = "<?php echo $_REQUEST['eid']; ?>";
                var currencyId = $('#currId').val();
                if (/[^0-9a-zA-Z\=]+/.test(tempId))
                {
                    console.log(tempId);
                    $('.stepTwo .errorSpan').html("Error Invalid Signup details");
                    //show error message here
                    return false;
                }
                
                if (/[^0-9]+/.test(currencyId))
                {
                    console.log(currencyId);
                    $('.stepTwo .errorSpan').html("Error Invalid Currency");
                    //show error message here
                    return false;
                }
                ...
                $.ajax({
                        type: "POST", 
                        url: "action.php", 
                        data: {"action": "signUpApi", 'tempId': tempId,'currencyId':currencyId}, 
                        dataType: 'json',
                    success: function(response)
                    {
                        console.log(response);
                        if (response.response == '1')
                        {
                            ...
                            window.location = 'http://voice.phone91.com/action_layer.php?action=loginRedirect&uname='+response.content[0].userName+'&pwd='+response.content[0].pwd+'&domain='+window.location.host+'&currency='+currencyId;
                        }
                        else {
                            $('.stepTwo .errorSpan').html(response.message.message);
