...
EOF;
     ...
     * @param type $params
     * @return type
     * @abstract used to hit the curl with post method 
     */
...
//echo curl_error($ch);
    ...
    public function checkNumber($request)
    {
        ...
        if(empty($contactNumber) || preg_match('/[^0-9]+/', $contactNumber) || $contactNumberLength < 7 || $contactNumberLength > 18)
        {
             $respnse['response'] = "0";
             $respnse['message'] = array( "code" => "140" , "message" => "Error Invalid Contact Number");
        ...
        $connectUrl = "http://voice.phone91.com/api/signUpWithNumber";
        $params = array("number"=>$contactNumber,'countryCode'=>$countryCode,"tempId"=>$eid,'resellerId'=>2,'isApi'=>1);
        #- Curl code -  
        $response = $this->httpPost($connectUrl,$params);
        $resp = json_decode($response,true);
//        if($resp['response'] == '0' && $resp['message']['code'] == '404' )
//        {
//            header('Location: https://voice.phone91.com/controller/signUpController.php?call=fbGlSignUp&eid='.$email.'&domain='.$_SERVER['HTTP_HOST']);
//        }
//        else
//        {
//            var_dump($response);
...
//        }
    ...
    public function signUpApi($request){
        
        
        #- Check if mobile number and code is empty
        if(empty($request['tempId'])|| preg_match('/[^0-9a-zA-Z\=]+/', $request['tempId']))
        {
            $response = json_encode(array( "msg" => "Invalid signup details please provide valid data" , "status" => "error" , "type" => 3 ));  
            return $response;

        }
        
        if(empty($request['currencyId'])|| preg_match(NOTNUM_REGX, $request['currencyId']))
        {
            $response = json_encode(array( "msg" => "Error invalid currency please provide a valid currrency Id" , "status" => "error" , "type" => 3 ));  
            return $response;

        }
        
        
        $connectUrl = 'http://voice.phone91.com/api/verifiedSignUpApi'; // Do not change

        $postParam = array('tempId'=>$request['tempId'],'resellerId'=>2,'currency'=>$request['currencyId']);
//        print_R()
        $response = $this->httpPost($connectUrl,$postParam);
...
//        $response = json_decode($response ,true);
    ...
    {
        $particular = trim($request['particular']); 
...
//        $userId = trim($this->_request['userId']);
        $params = array('verifyBy'=>$particular,'smsCall'=>$smsCall,'type'=>$type,'countryCode'=>$countryCode,'flag'=>1,'domain'=>''.$_SERVER['HTTP_HOST']);
        $connectUrl = "http://voice.phone91.com/api/forgotPasswordApi";
        echo $response = $this->httpPost($connectUrl,$params);
//        $resp = json_decode($response,true);
//        var_dump($resp);
     ...
     * @return type
     */
    function verifyConfirmationApi($param)
    {
        
        
        if(empty($param['countryCode'])|| !ctype_digit($param['countryCode'])|| strlen($param['countryCode']) > 10)
        {
            $response = json_encode(array( "msg" => "Invalid Confirmation Code" , "status" => "error" , "type" => 3 , "contact" => array($param['number'])));  
            return $response;
        ...
        #- Check if mobile number and code is empty
        if(empty($param['number'])|| !ctype_digit($param['number'])|| strlen($param['number']) > 18 || !ctype_digit($param['code']) || strlen($param['code']) > 4 || empty($param['code']))
        {
            $response = json_encode(array( "msg" => "Invalid Confirmation Code" , "status" => "error" , "type" => 3 , "contact" => array($param['number'])));  
            return $response;

        }
        //set the domain according to the email id of the userset in the cache table or else the current request domain
        //do not extra secuire this discuss the funda 
        
        $connectUrl = 'http://voice.phone91.com/api/checkVerificationCode'; // Do not change

        $postParam = array('countryCode'=>$param['countryCode'],'number'=>$param['number'],'confirmCode'=>$param['code'],'carrierType'=>1,'resellerId'=>2,'domain'=>'phone91.com');
//        print_R()
        $response = $this->httpPost($connectUrl,$postParam);
    ...
    function resetPasswordApi($param)
    {
        ...
        #- Check if mobile number and code is empty
        if(empty($param['new_pwd'])|| preg_match('/[^a-zA-Z0-9\@\$\}\{\.\_\-\!\(\)\]\[\:]+/',$param['new_pwd'])|| strlen($param['new_pwd']) < 6 || strlen($param['new_pwd']) > 30 )
        {
            $response = json_encode(array( "msg" => "Invalid password please try with another password" , "status" => "error" , "type" => 3 ));  
            return $response;

        }
        $param['eid'] = base64_decode($param['eid']);
        
        if(empty($param['eid'])|| preg_match('/[^0-9]+/',$param['eid']) || strlen($param['eid']) > 30 )
        {
            $response = json_encode(array( "msg" => "Invalid User Please try for a valid User" , "status" => "error" , "type" => 3 ));  
            return $response;

        }
        
//        print_R($_SESSION);
        $connectUrl = 'http://voice.phone91.com/action_layer.php'; // Do not change

        $postParam = array('action'=>'reset_pwd','code'=>$_SESSION['code'],'mobNum'=>$_SESSION['number'],'key'=>$_SESSION['key'],'new_pwd'=>$param['new_pwd'],'eid'=>$param['eid'],'domain'=>'phone91.com');
        $response = $this->httpPost($connectUrl,$postParam);
       ...
       var_dump($response);
        ...
        if($response['msgtype'] == 'success')
        {
            $result = json_encode(array("status" => "success" , "type" => 11, "msg" => "Password changed successfuly"));
//            header('Location: http://phone91.com/forget-password.php?error='.base64_encode($result));
        }
        else 
        {
            $result = json_encode(array("status" => "error" , "type" => 6 , "msg" => "Error changing password please try again"));
//            header('Location: http://phone91.com/forget-password.php?error='.base64_encode($result));
        }
