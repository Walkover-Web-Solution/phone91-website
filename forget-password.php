        ...
        { 
            ...
            if(is_array($requestParam['contact']))
            {
                ...
                if($requestParam['type'] == '1')
                {
                    ...
                    {
                        $contactNumDetail = get_object_vars($val);
                        
                        if($requestParam['type'] == 2)
                        {
                            $mobileNumber = explode('-' , $contactNumDetail['number']);

                            $option.= "<option code='".$mobileNumber[0]."' value='".$mobileNumber[1]."'>".$contactNumDetail['number']."</option>";
                        }
                        else
                            $confirmMobileNumber = $contactNumDetail['number'];
                        
                        $userId = $contactNumDetail['userId'];
                    }
                }
                else
                {
                       ...
                       // foreach($val as $value )
                        {
                            if($requestParam['type'] == 2)
                            {
                                $number = $contactNumDetail['number'];
                                $option.= "<option code='".$mobileNumber[0]."' number = '".$contactNumDetail['number']."'  value='".$contactNumDetail['userName']."'>".$contactNumDetail['userName']."</option>";
                            }
                            else
                                $confirmMobileNumber = $contactNumDetail['number'];
                        }
...
<?php if(isset($requestParam['msg']) &&  !empty($requestParam['msg'])  && $requestParam['status']!='success'  ) { ?> <div class="error_red"> <?php echo $requestParam['msg']; ?></div><?php } ?>
...
<aside class="boxsize">
    ...
    <!--<form name="verifyNumber" class="verifyNumber" id="verifyNumber" onsubmit="sendDetail(this)" action="javascript:void(0)" > -->
         ...
         <?php if($type == 'EMAIL'){ ?>
                ...
                <p class="para16 mdf">
                    If you didn't receive the code yet, <br>
                     ...
                     request another one via <a href="<?php echo REDIRECT_FB_GOOGLE; ?>/controller/signUpController.php?smsCall=3&call=resendVerificationCode&verifiedNumber=<?php echo $confirmMobileNumber; ?>&userId=<?php echo $userId; ?>&countryCode=<?php echo $countryCode; ?>">Email</a>
                </p>
        ...
        <p class="para16 mdf">
            If you didn't receive the code yet, <br>
            request another one via <a href="<?php echo REDIRECT_FB_GOOGLE; ?>/controller/signUpController.php?smsCall=1&verifiedNumber=<?php echo $number; ?>&call=resendVerificationCode&userId=<?php echo $userId; ?>&countryCode=<?php echo $countryCode; ?>">SMS</a> or <a href="<?php echo REDIRECT_FB_GOOGLE; ?>/controller/signUpController.php?smsCall=2&verifiedNumber=<?php echo $number; ?>&call=resendVerificationCode&userId=<?php echo $userId; ?>&countryCode=<?php echo $countryCode; ?>">call</a>
        </p>
