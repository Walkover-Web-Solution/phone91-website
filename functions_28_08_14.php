...
//echo curl_error($ch);
    ...
    public function checkNumber($request)
    {
        ...
        $connectUrl = "http://testing.phone91.com/api/checkVerifiedNumber";
...
//        $userId = trim($this->_request['userId']);
        $params = array('verifyBy'=>$particular,'smsCall'=>$smsCall,'type'=>$type,'countryCode'=>$countryCode);
        $connectUrl = "http://testing.phone91.com/api/forgotPasswordApi";
        $response = $this->httpPost($connectUrl,$params);
        $resp = json_decode($response,true);
        var_dump($resp);
