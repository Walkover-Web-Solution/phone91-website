...
$(document).ready(function(){
    ...
    check_user_exist =function()						
    {
	    ...
	    if(u.length >=5 && u.length < 25 && uNameReg.test(u) == false && isNaN(u) == true) 		
	    {
                		
		$("#username").css({'background':'url(images/loading.gif) no-repeat','background-position':'right center'})
		$.ajax({type: "GET",url: "https://voice.phone91.com/action_layer.php?action=check_avail",data: { username: u},dataType: 'jsonp',
...
//				$("#username").focus();
   ...
   alert(result);
	    ...
	    {
                if(isNaN(u) == false){
                    var errorMessage = 'Please enter atleast one character.'; 
                }else
                if(uNameReg.test(u))
                {
	...
	if(emailReg.test(u)) 		
	{
	    ...
	    $.ajax({type: "GET",url: "https://voice.phone91.com/action_layer.php?action=check_email_avail",data: { email: u},dataType: 'jsonp',
	    success: function(msg)
	    { 
...
//			    $("#email").focus();
	...
	if(u == '' || u.length<7 || u.length > 25)
	{
	    $("#password").next().removeClass("error_green").addClass("error_red").html("Please enter valid password min 7 and max 25 Character Long");	
